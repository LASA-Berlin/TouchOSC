...
// 0 = t_none, 1 = t_page, 2 = t_label, 3 = t_led, 4 = t_toggle, 5 = t_push, 6 =
// t_fader, 7 = t_rotary, 8 = t_multitoggle, 9 = t_multipush, 10 = t_multifader,
// 11 = t_x

char UDP_send_buffer[ARRARY_SIZE]; // zu sendende Daten
char UDP_rcv_buffer[ARRARY_SIZE]; // hier kommen UDP-Daten an, koennte noch
    // private werden

unsigned long last_millis_1s;
unsigned long last_millis_100ms;
unsigned long last_millis_10ms; // Für Timer-Ticks gebraucht
...
TouchOSC::TouchOSC(int port) { initNetwork(port); }
...
void TouchOSC::read(String* controlTyp, int* parameternummer, int* idx1, int* idx2, int* val_1, int* val_2)
{
    ...
    if (packet_size) {
        ...
        parse_udp(packet_size, controlTyp, parameternummer, idx1, idx2, val_1,
            val_2);
        MON_MSG("controlTyp: ");
        MON_MSG(*controlTyp);
...
{ // Needs to be called regulary, for new inputs from
    // TouchOSC
...
String TouchOSC::extract_value(String data, char separator, int index)
{
    ...
    // OSC-Message mit Trenner "/": /<1>/<2>/<3>/..., da erstes Trennzeichen am
    // Anfang Text-Message mit Trenner "=": <0>=<1>
    int found = 0;
    int strIndex[] = { 0, -1 };
...
void TouchOSC::parse_udp(int packet_size, String* controlTypPtr,
    int* parameterPtr, int* idx1ptr, int* idx2ptr,
    int* value1ptr, int* value2ptr)
{
    ...
    // Position der Komma suchen und Anzahl der Argumente
    for (i = 0; (i <= packet_size) && (UDP_rcv_buffer[i] != 0x2C); i++)
        ; // ","
    idx = i;

    if (UDP_rcv_buffer[idx + 1] == 'f')
        arg_count_f++;
    if (UDP_rcv_buffer[idx + 1] == 'i')
        arg_count_i++;
    if (UDP_rcv_buffer[idx + 2] == 'f')
        arg_count_f++; // "ff"
    if (UDP_rcv_buffer[idx + 2] == 'i')
        arg_count_i++; // "ii"
    arg_count = arg_count_f + arg_count_i;

    if (arg_count) {
        // erster Teil des Strings gibt Typ des Controls an, z.B. "fader" bei
        // "/fader/1000/9"
...
int TouchOSC::setup_udp_send_buffer(int type, int param, int idx_1, int idx_2,
    int val_1, int val_2)
{
...
void TouchOSC::initNetwork(int udpport)
{
    ...
    if (eePromData.flag != 0x55) {
        ...
        write_eeprom();
        MON_MSGLN(
            "WiFi name reset to SSID 'Make TouchOSC Bridge' and PW 'password'");
        SPIFFS.format();
    ...
    if (temp_ap_mode) {
        ...
        if (temp_ap_mode) {
            MON_MSGLN(
                "Station mode failed, will setup Acces Point Mode, IP 192.168.4.1");
            WiFi.mode(WIFI_AP); // Only Access point
            WiFi.softAP(eePromData.ssid, eePromData.password); // access point
        } else {
